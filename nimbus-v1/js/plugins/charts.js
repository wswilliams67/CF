...
(() => {
  ...
  /*
   * Fallback only. The real palette is --cnds-chart-color-1..10 in
   * plugins/charts.css, read off the chart element so it follows the theme —
   * see _paletteFrom(). These values are the previous Material Design defaults,
   * kept solely so a chart still draws if the stylesheet has not loaded.
   */
  const COLORS = [
    "#5ccc59",
    "#1976d2",
  ...
  /**
   * The series palette as the CHART sees it.
   *
   * Read from the element rather than :root, because the tokens are declared
   * on `.chart` and re-declared under [data-cnds-theme="dark"] .chart — asking
   * documentElement returns nothing and silently drops back to the fallback.
   */
  function paletteFrom(el) {
    ...
    try {
      ...
      for (var i = 1; i <= 10; i++) {
        var v = cs.getPropertyValue("--cnds-chart-color-" + i).trim();
        if (v) out.push(v);
      }
    } catch (e) { /* fall through to COLORS */ }
  ...
  class Chart extends NimbusComponent {
    constructor(element, config = {}) {
      ...
      this._svg = null;
      this._tooltip = null;
      /* Resolved AFTER the .chart class is applied — see below. The palette
         tokens are declared on .chart, so reading them before the class exists
         returns nothing and silently drops to the fallback. */
      this._colors = this._config.colors || null;
      this._init();
    }
    ...
    _init() {
      ...
      /* Now the element carries .chart, so --cnds-chart-color-* resolves —
         including any product override (.cnds-product-casefusion .chart). */
      if (!this._colors) this._colors = paletteFrom(this._element);
    ...
    _renderBar(width, height) {
      ...
      /*
       * STACKED bars segment one bar per label instead of standing several
       * side by side. Two things change and both are required — getting only
       * one gives a chart that is wrong rather than merely ugly:
       *
       *   the SCALE is the tallest COLUMN TOTAL, not the tallest single value.
       *     Scaling to the largest segment would push every stack off the top.
       *   the BAR is the full group width, not a share of it, because there is
       *     one bar per label rather than one per dataset.
       *
       * Use it where the series are parts of a whole and the total is the
       * point. Where the series are independent, grouped bars stay correct —
       * a stack of unrelated quantities invites reading a total that means
       * nothing.
       */
      var stacked  = !!this._config.stacked;
      var allValues = [];
      if (stacked) {
        labels.forEach(function (_, idx) {
          var sum = 0;
          datasets.forEach(function (ds) { sum += (ds.data[idx] || 0); });
          allValues.push(sum);
        });
      } else {
      ...
      var barW       = stacked
        ? groupWidth * this._config.barWidth
        : (groupWidth * this._config.barWidth) / datasets.length;
      var self       = this;

      /* Running height per label, so each segment sits on the one below it. */
      var stackTops = labels.map(function () { return 0; });
      ...
      datasets.forEach(function (ds, dsIdx) {
        ...
        labels.forEach(function (label, idx) {
          ...
          var x         = stacked
            ? p.left + groupWidth * idx + (groupWidth - barW) / 2
            : p.left + groupWidth * idx + (groupWidth - barW * datasets.length) / 2 + barW * dsIdx;
          var y         = p.top + chartH - barH;
          if (stacked) {
            /* Sit on the accumulated height, then add to it for the next
               dataset. A zero segment contributes nothing and takes no space,
               which is what lets a key type that never fires stay in the
               legend without drawing a sliver. */
            y = p.top + chartH - stackTops[idx] - barH;
            stackTops[idx] += barH;
          }
          ...
          rect.setAttribute("y",      self._config.animated ? (stacked ? y + barH : p.top + chartH) : y);
          rect.setAttribute("width",  stacked ? barW : barW - 2);
          rect.setAttribute("height", self._config.animated ? 0 : barH);
          rect.setAttribute("fill",   fillColor);
          rect.setAttribute("rx",     stacked ? "0" : "2");
          if (bdColor) {
          ...
          if (self._config.animated) {
            ...
            animY.setAttribute("from",          stacked ? (y + barH) : (p.top + chartH));
    ...
    _createLegendItem(label, color, textColor) {
      ...
      /* Layout comes from .chart-legend-item in plugins/charts.css, NOT from
         an inline copy. It was duplicated here, and inline beats the
         stylesheet — so the CSS rule was dead and spacing could only be
         changed in JS. Only the swatch COLOUR stays inline, because it is
         per-series data rather than styling. */
